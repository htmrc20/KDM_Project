		...
		botReply = function (sender_id, text, data) {
			...
			request.done(function(response) {
			  ...
			  response.forEach(function(item, index, arr) {
                ...
                if (hr_name === '') $('p.error').html('Tia is typing...');
					...
					if (payload !== undefined) {
						...
						payload.forEach(function(pItem) {
						    if ($.inArray(pItem.payload, ["Financial Markets", "Technology Services", "Digital", "Customer Operations", "CLX"]) != -1){
							    answers += "<div class='payload_eclerx' data-value='" +pItem.payload + "'><span data-value='" +pItem.payload + "' data-type='" +pItem.title+ "'>" + pItem.title + "</span><p style='' class='verticals_know_more' data-link='"+ vertical_links[pItem.payload.toLowerCase()] +"' target='_blank'>Click to know more</p></div>";
						    } else {
...
//                })
        ...
        $("#chat-circle").click(function() {
            ...
            if($(this).css('display') == 'block' && is_initialized == true) {
                ...
                $('.message_input_wrapper').removeAttr('disabled');
                $('.message_input_wrapper').css('background', '#fff');
                is_initialized = false;
            }
        });
...
//                return false;
//            }
			clearTimeout(autoCloseTimeOut); // added by Meenakshi on 23rd July
            $("#chat-circle").toggle('fade');
            $(".chat-box").toggle('fade');
